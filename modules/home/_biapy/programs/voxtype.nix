/**
  ...
  Voxtype provides voice-to-text with push-to-talk for Wayland compositors.

  ## 🛠️ Tech Stack

  - [Voxtype homepage](https://voxtype.io/)
    ([Voxtype @ GitHub](https://github.com/peteonrails/voxtype)).
  - [dotool @ Sourcehut](https://git.sr.ht/~geb/dotool).
...
let
  ...
  cfg = config.biapy.programs.voxtype;
in
{
  options = {
    biapy.programs.voxtype = {
      enable = mkEnableOption "Voxtype";
    };
  };

  config = mkIf cfg.enable {
    # voxtype requires quickshell
    programs.quickshell.enable = mkDefault true;
    ...
    home = {
      ...
      file = {
        ...
        "${config.xdg.dataHome}/voxtype/models/ggml-base.bin".source = mkDefault (fetchurl {
          url = "https://huggingface.co/ggerganov/whisper.cpp/resolve/80da2d8bfee42b0e836fc3a9890373e5defc00a6/ggml-base.bin";
          hash = "sha256-YO1bw90U7qhWST0zQ0m0BXgt3K8AKNS130CINF+6Lv4=";
        });

        "${config.xdg.configHome}/voxtype/config.toml".text = mkDefault ''
